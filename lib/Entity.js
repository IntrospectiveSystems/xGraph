...
module.exports.Entity = class Entity {
	constructor(nxs, imp, par, log) {
		...
		//TODO: Move this to a better place when Entity is refactored
		if ('Log' in par) {					
			let Logger = require('./Logger.js');
			let config = par.Log;
			let baseOptions = log.__options;	
			for (let key in baseOptions) {
				if (!(key in config)) {
					config[key] = baseOptions[key];
				}
			}
			if (config.scope) {
				switch(config.scope) {
					case '$name':
						config.scope = this.instance.Par.Name
						break;
					case '$module':
						config.scope = this.instance.Par.Module
						break;
					default:
				}
			}			
			this.instance.log = new Logger(config);
			delete this.instance.Par.Log;
		} else {
	...
	async getServices(nxs, services) {		
		...
		if(services === undefined) {
			if('Services' in this.Par) {
				...
				this.log.e(`Error in ${this.Par.Entity}`);
				this.log.e('No service providers available');
				return;
			}
		} else {
			srvcs = services;
