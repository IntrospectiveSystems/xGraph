...
module.exports.Entity = class Entity {
	constructor(nxs, imp, par, log) {
		...
		// okay, simple explanation. because we're doin startup here and not
		// out in nexus, in phases, we need to at least wait until Nexus is
		// ready. before hand, nexus called it, so it just did it when it saw
		// fit. now its internal, we wait for a message.
		// semaphore is the promise that we await, startupProcedure is what Nexus
		// will call on each entity when ready.
		// and lastly, StartupProcedurePromise, is what our dispatch waits for.
		let StartupProcedure;
		let StartupProcedureSemaphore = new Promise(resolve => {
			StartupProcedure = _ => {
				resolve();
			}
		});

		let StartupProcedurePromise = new Promise(async (resolve) => {
			// wait for start signal
			await StartupProcedureSemaphore;
			await new Promise(res => {
				if('$Setup' in par) this.dispatch.call(this, {Cmd: par['$Setup']}, _ => {
					res();
				});
				else res();
			});
			resolve();
			await new Promise(res => {
				if('$Start' in par) this.dispatch.call(this, {Cmd: par['$Start']}, _ => {
					res();
				});
				else res();
			});
			// delete context.StartupProcedure;
			// delete context.StartupProcedurePromise;
			// delete context.StartupProcedureSemaphore;
		});
		...
		this.instance.dispatch = this.dispatchInternal.bind(this.instance);
		...
		this.startupProcedure = StartupProcedure;
		this.startupProcedurePromise = StartupProcedurePromise;
	}
	 ...
	 * @callback fun
	 * this dispatch happens on the entity level, as opposed to instance level.
	 */
	async dispatch(com, fun = _ => _) {
		if('Passport' in com && com.Passport.To !== com.Passport.From) await this.startupProcedurePromise;
		await this.instance.dispatch(com, fun);
	}

	// this function is mapped to this.instance.dispatch
	dispatchInternal(com, fun) {
		try {
			if (com.Cmd in this) {
