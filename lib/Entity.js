...
module.exports.Entity = class Entity {
	...
	dispatch(com, fun = _ => _) {
		try {
			...
			// Silence for now, need to log only for direct sends and not broadcasts
	 ...
	 * @callback fun
	 */
	deleteEntity(nxs, pid, fun) {
		nxs.deleteEntity(pid, fun);
	}
	 ...
	 * @param {object} nxs - The nexus context
	 * @param {object} command - The command object to broadcast
	 * @param {string} command.Cmd - The command name to broadcast
	 * @param {boolean} useWebSocket - If true, send through WebSocket; if false, use local Nexus broadcast
	 * @param {Function} fun - Optional callback function
	 */
	broadcast(nxs, command, fun, useWebSocket = false,) {
		// Handle case where useWebSocket parameter is omitted and fun is passed as second parameter		
		...
		// Use local Nexus broadcast (default behavior)
		this.log.x(`Broadcasting command ${command.Cmd} through local Nexus`);
		
		// Clean the command to avoid reference issues
		const cleanCommand = JSON.parse(JSON.stringify(command));
		
		// Set the from field
		if (!cleanCommand.Passport) {
			cleanCommand.Passport = {};
		}
		cleanCommand.Passport.From = this.Par.Pid;
		if (useWebSocket) {
			cleanCommand.Passport.UseWebSocket = true;
		}
		
		// Use Nexus broadcast functionality
		nxs.broadcast(cleanCommand, fun);
	}
	...
	/**
	 * Get a value from the entity's context.
	 * @param {object} com - The command object containing the key to retrieve.
	 * @param {string} com.Val - The key to retrieve, supports dot notation for nested keys.
	 * @returns {any} The value associated with the key, or undefined if not found.
	 * @callback fun
	 */
	_get(com, fun) {
		let err = null;
		// Support dot notation for deep object key resolution
		if (com.Val.includes('.')) {
			const keys = com.Val.split('.');
			let current = this;
			...
			for (const key of keys) {
				if (current && typeof current === 'object' && key in current) {
					...
					err = `Key path not found: ${com.Val}`;
					// Key path not found
					fun(err, undefined);
					return;
				}
		...
		// Original direct property access
		if (com.Val in this) {
			fun(null, this[com.Val]);
			return;
		}
		
		// Property not found
		err = `Key not found: ${com.Val}`;
		fun(err, undefined);
	}


	/**
	 * Log Interface allows for external changes to be made to the entity's logging system.
	 * Scans the com object for registered operations and executes them on this entity's Log instance.
	 * @param {object} com - Command object containing log operations as properties
	 * @param {function} fun - Callback function
	 */
	_logInterface(com, fun) {
		try {
			const results = {};
			const errors = [];

			// Define available log operations for the entity's Log instance
			const logOperations = {
				setLogLevel: (value) => {
					if (this.log && typeof this.log.setLogLevel === 'function') {
						this.log.setLogLevel(value);
						return { success: true, operation: 'setLogLevel', value: value };
					} else {
						throw new Error('setLogLevel not available on log instance');
					}
				},				

				getLocalLogLevels: () => {
					if (this.log && this.log.localLogLevels) {
						return { success: true, operation: 'getLocalLogLevels', levels: this.log.localLogLevels };
					} else {
						throw new Error('localLogLevels not available on log instance');
					}
				},

				getLocalLogTypes: () => {
					if (this.log && this.log.localLogTypes) {
						return { success: true, operation: 'getLocalLogTypes', types: this.log.localLogTypes };
					} else {
						throw new Error('localLogTypes not available on log instance');
					}
				},

				enableLocalLogLevel: (levelName) => {
					if (this.log && this.log.localLogLevels) {
						if (typeof levelName === 'string' && levelName in this.log.localLogLevels) {
							this.log.localLogLevels[levelName] = true;
							return { success: true, operation: 'enableLocalLogLevel', level: levelName };
						} else {
				...
				disableLocalLogLevel: (levelName) => {
					if (this.log && this.log.localLogLevels) {
						if (typeof levelName === 'string' && levelName in this.log.localLogLevels) {
							this.log.localLogLevels[levelName] = false;
							return { success: true, operation: 'disableLocalLogLevel', level: levelName };
						} else {
							throw new Error(`Invalid log level: ${levelName}`);
						}
					} else {
				...
				getScope: () => {
					if (this.log && this.log.scope !== undefined) {
						return { success: true, operation: 'getScope', scope: this.log.scope };
					} else {
						throw new Error('scope not available on log instance');
					}
				}
			};

			// Scan com object for registered operations
			for (const [operationName, operationHandler] of Object.entries(logOperations)) {
				if (operationName in com) {
					try {
						const result = operationHandler(com[operationName]);
						results[operationName] = result;
					} catch (error) {
						errors.push({
							operation: operationName,
							error: error.message,
							value: com[operationName]
						});
					}
				}
			}
			...
			const response = {
				...
				results: results,
				errors: errors,
				processedOperations: Object.keys(results).concat(errors.map(e => e.operation))
			};

			if (fun) {
				fun(errors.length > 0 ? errors : null, response);
			}
			return response;
			...
			if (fun) {
				fun(error.message, errorResponse);
			}
