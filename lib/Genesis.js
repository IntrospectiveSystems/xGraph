...
function genesis(__options = {}) {
	...
	return new Promise(async (resolveMain, rejectMain) => {
		...
		try {
			...
			try {
				tempConfig = JSON.parse(fs.readFileSync(Params.config || Path.join(CWD, 'config.json')));
			} catch (e) {
				log.e('Specified configuration file does not exist or is in an unparsable format.');
				rejectMain('Specified configuration file does not exist or is in an unparsable format.');
				return;
			}

			let systemTemplate = await setup(tempConfig, true);
			let HalfCache = systemTemplate.HalfCache;
			let Config = systemTemplate.Config;

			if (!checkFlag('half')) await genesis();
		} catch (e) {
			rejectMain(e);
			return;
		}
		 ...
		 * and clean the cache.
		 */
		async function setup(tempConfig, root = true) {
			...
			return new Promise(async (resolveSetup, rejectSetup) => {
					...
					for (let ifolder = 0; ifolder < moduleKeys.length; ifolder++) {
						modArray.push(new Promise((res, rej) => {
							...
							GetModule(modrequest, async function (err, mod) {
								if (err) {
									log.e(`Failed to retreive ${folder}`);
									rej(`Failed to retreive ${folder}`);
								} else {
				...
				async function processApexPar(apx, inst, processPidReferences) {
					...
					async function symbolPhase2(val) {
						...
						if (val.toLocaleLowerCase().trim() == '@system') {
							let path, config;
							try {
								...
								let systemObject = await setup(config);
									...
									zip.loadAsync(cacheBuffer).then(async (a) => {
										...
										for (let key in a.files) {
											...
											for (let bowerModuleName in dependencies) {
												...
												// packageArray = ['PolymerVis/monaco-editor#1.0.0',
												// 'jquery#^3.0.0']
												await new Promise(res => {
													proc.execSync('bower install "--config.directory='
														+ `${Path.join(__options.cwd, 'Static',
		...
		async function genesis() {
			...
			//somewhere need to add the modules to the cache from the Module zip i imagine
			await new Promise(res => {
			...
			//then we need to add the apex instances to it
			cacheInterface.createInstance(inst, apx);
