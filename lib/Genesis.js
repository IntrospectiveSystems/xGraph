...
function genesis(__options = {}) {
	...
	const {retrieveModule} = require('./broker.js')
	let cacheInterface;
			...
			return new Promise(async (resolveSetup, rejectSetup) => {
				...
				async function retrieveModules(modules) {
					...
					for(let moduleName in modules) {
						log.v(modules[moduleName]);
						promises.push(new Promise(async (res) => {
							log.v(Config.Sources[modules[moduleName].Source]);
							ModCache[moduleName] = await retrieveModule(
								Config.Sources[modules[moduleName].Source],
								moduleName,
								modules[moduleName].Version || undefined);
							res();
						}));
					}
					await Promise.all(promises);
					// console.dir(ModCache)
					// process.exit(0)
				}
				...
				async function processApexPar(apx, inst, processPidReferences) {
					...
					async function symbolPhase2(val) {
						...
						if (directive == '@system') {
							...
							if (!fs.existsSync(path)) { //TODO fix fs
								rejectSetup(`Specified configuration file does not exist: ${path}`);
								return;
							}
							try {
								tempConfig = JSON.parse(fs.readFileSync(path)); //TODO fix fs
							} catch (e) {
