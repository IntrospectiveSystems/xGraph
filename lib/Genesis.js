...
function genesis(__options = {}) {
	...
	const logger = checkFlag('logger') ? __options.logger : new Logger(__options);
	...
	return new Promise(async (resolveMain, rejectMain) => {
					...
					async function symbolPhase2(val) {
						...
						if (directive == '@system') {
							...
							try {
								...
								// Build Apex List from the spawned system's modules
								const apexList = {};
								if (config.Modules) {
									for (let moduleName in config.Modules) {
										if (moduleName !== 'Deferred') {
											// Populate base Apex List
								...
								// Return spawn information with Apex List as Modules instead of Sources/Modules
								return {
							...
							switch (key) {
								...
								case '@config': {
									...
									try {
										...
										symbolPhase0(valJSON);
										return symbolPhase3(valJSON);
									} catch (err) {
							...
							itemPromises.push(new Promise(async (resolve) => {
								...
								if (fs.lstatSync(curPath).isDirectory()) {
									...
									fs.readFile(curPath, function (err, data) {
										// log.x(curPath.length > 80 ? curPath.substr(0, 35) 
		...
		async function genesis(system) {
			...
			if(checkFlag('node_modules')) {
				let nodeModulesPath = checkFlag('node_modules');
