					...
					async function symbolPhase2(val) {
						...
						if (directive == '@system') {
							...
							try {
								...
								log.e('Specified configuration file does not exist ' +
									'or is in an unparsable format.');
								rejectSetup('Specified configuration file does not exist ' +
									'or is in an unparsable format.');
								return;
							}
							...
							try {
								...
								log.e(`Error processing subsystem at: ${path}`);
		...
		async function genesis(system) {
			...
			*/
			function cleanCache() {
				...
				if (__options.state == 'development' && fs.existsSync(CacheDir)) {
			...
			async function cacheModules(ModCache) {
				...
				for (let folder in ModCache) {
					ModulePromiseArray.push(await cacheInterface.addModule(folder, ModCache[folder]));
				}
