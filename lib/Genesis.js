...
async function processSymbolPhase1(val, Apex, log, rejectSetup, skipSymbol = false) {
	...
	if (typeof val === 'object') {
		if (Array.isArray(val)) {
			...
			// Check if this object has skipInfo metadata (from Phase0 decorator processing)
			const skipInfo = val.__skipInfo || {};

			for (let key in val) {
				if (key === '__skipInfo') continue; // Skip the metadata itself

				// Check if this specific property has skip flags
				const keySkipInfo = skipInfo[key] || {};
				const skipAllProcessing = keySkipInfo.skipAll || false;
				const propSkipSymbol = keySkipInfo.skipSub || false;

				// If !! decorator, skip all processing for this property
				if (skipAllProcessing) {
					continue;
				}

				// Process the property value
				// If ! decorator on this property, enable skipSymbol for its value
				// Otherwise inherit the parent's skipSymbol setting
				const effectiveSkipSymbol = propSkipSymbol || skipSymbol;
				val[key] = await processSymbolPhase1(val[key], Apex, log, rejectSetup, effectiveSkipSymbol);
			}

			// Don't clean up __skipInfo yet - Phase2 and Phase3 still need it
		}
		return val;
	}
	...
	if (typeof val === 'object') {
		if (Array.isArray(val)) {
			...
			for (let key in val) {
				...
				// Otherwise inherit the parent's skipSymbol setting
				const effectiveSkipSymbol = propSkipSymbol || skipSymbol;
				val[key] = await processSymbolPhase2(val[key], Params, CWD, __options, log, rejectSetup, effectiveSkipSymbol);
			}

			// Don't clean up __skipInfo yet - Phase3 still needs it
		}
		return val;
	}
...
async function processSymbolPhase3(val, inst, Params, CWD, __options, log, skipSymbol = false, Apex = {}, skipNestedSymbols = false) {

	if (typeof val === 'object') {
		if (Array.isArray(val)) {
			val = await Promise.all(val.map(v => processSymbolPhase3(v, inst, Params, CWD, __options, log, skipSymbol, Apex, skipNestedSymbols)));
		} else {
			// Check if this object has skipInfo metadata (from Phase0 decorator processing)
			const skipInfo = val.__skipInfo || {};
			...
			for (let key in val) {
				...
				const keySkipInfo = skipInfo[key] || {};
				const skipAllProcessing = keySkipInfo.skipAll || false;
				const propSkipNestedSymbols = keySkipInfo.skipSub || false;
				...
				// Process the property value
				// If ! decorator on this property, enable skipNestedSymbols for its value
				// Otherwise inherit the parent's skipNestedSymbols setting
				const effectiveSkipNested = propSkipNestedSymbols || skipNestedSymbols;
				val[key] = await processSymbolPhase3(val[key], inst, Params, CWD, __options, log, skipSymbol, Apex, effectiveSkipNested);
			}

			// Clean up the metadata
			if (val.__skipInfo) {
				delete val.__skipInfo;
			}
		}
		return val;
	}
	...
	// If skipSymbol is true, don't process @ directives
	// This preserves @ directives as literal strings in nested content
	if (skipSymbol) {
		return val;
	}
	...
	if (val.charAt(0) === '@') {
		...
		// Apply macro replacement to the directive value (e.g., {SymbolTest}/test.json)
		val = replaceMacros(val, __options);
		...
		switch (key) {
			...
			case '@config': {
				...
				try {
					...
					// Always run Phase0 to process macros and decorators in the loaded file
					valJSON = processSymbolPhase0(valJSON, __options, false);

					// Apply phase 1 ($ symbol resolution) if not skipping nested symbols
					if (!skipNestedSymbols) {
						valJSON = await processSymbolPhase1(valJSON, Apex, log, () => {}, false);
					}

					// When recursively processing loaded content, use skipNestedSymbols as the skipSymbol flag
					// This ensures nested @ and $ directives are preserved as literal strings
					return processSymbolPhase3(valJSON, inst, Params, CWD, __options, log, skipNestedSymbols, Apex, skipNestedSymbols);
				} catch (err) {
...
async function processApexPar(apx, inst, processPidReferences, Apex, Params, CWD, __options, log, rejectSetup) {
	...
	if (inst.Par && typeof inst.Par === 'object') {
		...
		for (let key in inst.Par) {
			...
			// Process the immediate value normally (skipSymbol=false),
			// but pass skipSubProperties to control nested symbol processing
			inst.Par[key] = await processSymbolPhase3(inst.Par[key], inst, Params, CWD, __options, log, false, Apex, skipSubProperties);
		}
