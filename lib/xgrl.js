...
async function getProtocolObject(xgrl, __options) {
	const logger = (('logger' in __options) && (__options['logger'])) ? 
		__options.logger : new Logger(__options);
	const log = new Log(logger, 'xgrl');
...
function getProtocolModule(protocol, __options) {
	...
	return new Promise(function (resolve, reject) {
		...
		if (fs.existsSync(cacheFilepath)) {
			log.x(`Returning ${protocol} from ${cacheFilepath}`);
		...
		log.x(`Retrieving: ${protocol} from protocols.xgraphdev.com`);
		...
		let req = https.request(options, function (res) {
			...
			res.on('end', _ => {
				try {
					log.x(`Returning ${protocol} from protocols.xgraphdev.com`);
