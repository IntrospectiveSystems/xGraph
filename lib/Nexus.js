...
module.exports = function xGraph(__options = {}) {
	...
	this.boot = async function boot() {
		const __options = this.__options;

		const log = checkFlag('logger') ?
			__options.logger :
			createLogger(__options);
		let cacheInterface;
		...
		log.v(`cache folder set to ${__options.cache}`);
		...
		async function initiate() {
			...
			await cacheInterface.startup;

			if (cacheInterface.wroteManifest) {
				// if the cache had to write a manifest, there was no cache there...
				cacheInterface.delete();
				//yeah we should probably put all our custom errors in their own class and make them real
				//TODO that ^^^^^
				throw(new (class ENoCache extends Error {
					constructor(path) {
						super(`No cache exists at ${path}`);
						this.name = 'E_NO_CACHE';
					}
				})(__options.path));
			}
			...
			async function setup() {
				...
				for (let pid in startupPids) {
					...
					let context = await getEntityContext(pid);
					context.lock(context => {
						context.startupProcedure();
						return context;
					});
		...
		/**
			 ...
			 * @callback fun				the callback to return the pid of the generated entity to
			 */
		async function getFile(module, filename, fun) {
			let mod = ModCache[module];
			if (filename in mod.files) {
				let dat = await mod.file(filename).async('string');
			...
			// log.e(err);
			if(fun) fun(err);
			// only really throw if the user meant to use promises
			// or forgot a callback
			// otherwise that could cause an issue.
			else throw err;
		}
		...
		/**
			 ...
			 * @param {object} inst        its a module definition (except sometimes its not i guess? idfk)
