...
module.exports = function xGraph(__options = {}) {	
	...
	this.boot = function boot() {
		return (async function (__options) {
			...
			const logger = new Logger(__options);
			logger.initialize();
			const log = new Log(logger, 'Nexus');
			
			// Initialize WebSocket server if spawned
			let webSocketServer = null;
			if (process.env.XGRAPH_SPAWNED === 'true' && process.env.XGRAPH_WEBSOCKET_PORT) {
				try {
					const WebSocketServer = require('./WebSocketServer.js');
					webSocketServer = new WebSocketServer({
						port: parseInt(process.env.XGRAPH_WEBSOCKET_PORT),
						log: log,
						nexus: null  // Will be set after Nxs object is fully constructed
					});
					
					log.i('Initializing WebSocket server for spawned xGraph system');
				} catch (error) {
					log.e('Failed to initialize WebSocket server:', error);
				}
			}
			...
			function indirectEvalImp(entName, entString, ...injections) {
				...
				if(typeof imp == 'undefined') {
					if (!('dispatch' in imp)) {
						log.e('Invalid Entity File: ', entName);
			...
			function SendProfileData() {			
				// TODO Change to return from NedxusSystem info websocket command
				/*
				oldProfile({					
					Type: 'System',
			...
			if (webSocketServer) {
				try {
					...
					webSocketServer.setNexus(Nxs);
					
					// Handle WebSocket routing for module calls
					webSocketServer.on('moduleResponse', (response) => {
						// Module responses are handled internally by WebSocket server
						// This event allows for additional processing if needed
						log.d('Module response received via WebSocket:', response);
					});
					
					log.i(`WebSocket server started on port ${webSocketServer.port}`);
				} catch (error) {
					log.e('Failed to start WebSocket server:', error);
			...
			async function initiate() {
				...
				let LifeCycles = ['PreInit', 'Init', 'PostInit', 'Setup', 'Link', 'Start', 'Engage'];
				let exitTaskList = ['Disconnect', 'Stop', 'Cleanup', 'Close'];
				...
				for (let task of exitTaskList) {
					exitTasks[task] = Object.assign({}, cache[task]);
				}				

				await runLifeCycles(LifeCycles);
				...
				async function runLifeCycles(stages) {					
					for (let stage of stages) {
						await runLifeCycle(stage);
					}						
				}


				async function runLifeCycle(stage) {
					log.i(`--Nexus/${stage}`);

					let lifeCycle = cache[stage];
					
					let lifeCycleArray = [];

					for (let pid in lifeCycle) {
						lifeCycleArray.push(new Promise((resolve, _reject) => {
							let com = {};
							com.Cmd = lifeCycle[pid];
							com.Passport = {};
							com.Passport.To = pid;
							com.Passport.Pid = genPid();
					...
					await Promise.all(lifeCycleArray);
					log.x(`--Nexus: LifeCycle ${stage} Complete`);
				}
			...
			async function exit(code = 0) {
				log.i('--Nexus/Stop with exit code ' + code);
				
				// Stop WebSocket server if running
				if (webSocketServer) {
					log.x('Stopping WebSocket server as part of Nexus shutdown process');
					try {
						await webSocketServer.stop();
						log.i('WebSocket server stopped');
					} catch (error) {
						log.e('Error stopping WebSocket server:', error);
					}
				}
				
				log.x('Running exit tasks for all modules in the system');
				let tasks = ['Disconnect', 'Stop', 'Cleanup', 'Close'];
				for (let task of tasks) {
					await runCloseTask(task, exitTasks[task]);
					log.x(`--Nexus: ${task} Complete`);
				}

				function runCloseTask(task, pids) {
					let tasks = [];
					for (let pid in pids) {
						tasks.push(new Promise((resolve, _reject) => {
							let com = {};
							com.Cmd = task;
							com.Passport = {};
							com.Passport.To = pid;
			...
			function sendMessage(com, fun = _ => _) {
				...
				if (webSocketServer) {
					webSocketServer.broadcast(profileMessage);
				}				
				...
				async function done(err, entContextVolatile) {
					...
					if ((EntCache[pid].Apex == EntCache[pid].Pid) || (entApex == apx)) {
						...
						//log.i('Ent Context for PID ' + pid + ':', entContext);
				...
				function reply(err, q) {
					if (webSocketServer) {
						profileMessage.Profile.Callback = true;
						webSocketServer.broadcast(profileMessage);
					}					
					fun(err, q);
				}
			...
			/**
			 * Get a list of all active modules with their names and apex PIDs
			 * @returns {Promise<object>} - Promise that resolves to object with module names as keys and apex PIDs as values
			 */
			async function getModuleList() {
				const moduleList = {};
				
				// Iterate through all entities in cache to find apex entities (modules)
				for (const pid in EntCache) {
					//log.i(`Checking entity ${pid} for module list inclusion`);
					try {
						//log.i(`Checking entity ${pid} for module list inclusion`);
						// Get entity context from volatile wrapper
						const entityContext = await new Promise(res => 
							EntCache[pid].lock((context) => {
								res(context);
								return context;
							})							
						);						
						
						let entityContextPar = entityContext.instance.Par;						

						// Check if this entity is an Apex (main entity of a module)
						if (entityContextPar.Apex === entityContextPar.Pid) {
							// Get the module name from the entity's Par
							if (entityContextPar.Name || entityContextPar.Module) {
								const moduleName = entityContextPar.Name || entityContextPar.Module;
								moduleList[moduleName] = pid;
							}
						}
					} catch (error) {
						log.w(`Error accessing entity ${pid} for module list:`, error);
					}
				}

				log.w(`Module list generated with ${Object.keys(moduleList).length} modules found`);
				
				return moduleList;
			}

			/**
			 * Broadcast a command to all modules that have the specified command function
			 * @param {object} command - The command object containing Cmd and other parameters
			 * @param {string} command.Cmd - The command name to broadcast
			 * @param {Function} fun - Optional callback function
			 * @returns {Promise} - Returns array of results from all modules that handled the command
			 */
			async function broadcast(command, fun = _ => _) {
				if (!command || !command.Cmd) {
					const error = 'Broadcast requires a command object with Cmd property';
					log.w(error);
					fun(error);
					return [];
				}

				log.x(`Broadcasting command: ${command.Cmd} to all modules`);
				
				const results = [];
				const promises = [];

				if (!('Passport' in command)) {
					if (fun) { fun('No Passport in command for broadcast', command); }
					return;
				}

				if (command.Passport.UseWebSocket) {
					// Send broadcast command through WebSocket if specified in Passport
					log.x(`Broadcasting command ${command.Cmd} through WebSocket`);
					if (webSocketServer) {
						webSocketServer.broadcast(command);
					}
					if (fun) { fun(null, command)}
					return;
				}
				
				// Iterate through all entities in cache to find ones that can handle the command
				for (const pid in EntCache) {
					const entity = EntCache[pid];
					
					try {
						// Check if this entity is an Apex (main entity of a module)
						if (entity.Apex === entity.Pid) {
							// Create a copy of the command with proper routing
							...
							// Send the message and collect the promise
							const promise = new Promise((resolve) => {
								sendMessage(broadcastCommand, (err, result) => {
									if (err) {
										log.d(`Module ${pid} error for command ${command.Cmd}: ${err}`);
										resolve({ pid, error: err });
									} else {
										log.d(`Module ${pid} handled command ${command.Cmd}`);
										resolve({ pid, result });
									}
								});
							});
						...
						log.e(`Error broadcasting to ${pid}:`, error);
					}
				}
				...
				try {
					...
					log.x(`Broadcast complete. ${results.length} modules responded to ${command.Cmd}`);
					fun(null, results);
					return results;
				} catch (error) {
					log.e('Error during broadcast:', error);
					fun(error);
					return results;
				}
			}
			...
			async function genEntity(par, log, fun = _ => log.e(_)) {
				...
				Type: 'Entity',
				Modules: [
					{name: par.Module + '.' + par.Entity.split('.')[0], pid: par.Pid}
				]
				});*/
			 ...
			 */
			function deleteEntity(pid) {
				log.w('--Nexus/DeleteEntity: ', pid);
				...
				await EntCache[pid].lock((_entityContext) => {
					return new Promise((res, _rej) => {					
						...
						cacheInterface.getEntityPar(pid, (err, data) => {
							...
							let options = {};							
							options.scope = par.Name ? par.Name : par.Module;							
							options.pid = pid;
							let entityLog = new Log(logger, options.scope, options);
							//log.i(`--Nexus/GetEntityContext: ${par.Module}-${par.Entity.split('.')[0]}`);
			...
			async function genModule(moduleDefinition, fun = _ => _) {
				...
				//TODO:: Load this array from external config/defaults
				let LifeCycleStages = ['PreInit', 'Init', 'PostInit', 'Setup', 'Link', 'Start'];
				let ExitCycleStages = ['Disconnect', 'Stop', 'Cleanup', 'Close'];
				let LifeCycleObj = {};											
				let PromiseArray = [];
				let symbols = {};
				...
				for (let moduleKey in moduleDefinitions) {
					...
					PromiseArray.push(new Promise((res, _rej) => {
						...
						GetModule(inst.Module, async function (err, mod) {
							...
							for (let key in schema.Apex) {
								if (key.startsWith("$")) {
									let stage = schema.Apex[key].split(".")[0];									
									if (stage in LifeCycleObj) {
										LifeCycleObj[stage][pidapx] = schema.Apex[key];
									} else if (ExitCycleStages.includes(stage)) {
										exitTasks[stage][pidapx] = schema.Apex[key];
									}
								}
							}
							res();
				...
				log.i('Running LifeCycles');
				await runLifeCycles(LifeCycleStages);

				fun(null, ('Top' in symbols) ? symbols['Top'] : null, symbols);

				async function runLifeCycles(stages) {					
					for (let stage of stages) {
						log.i(`--Nexus/GenMod/${stage}`);
						await runLifeCycle(stage);
					}						
				}
				...
				async function runLifeCycle(stage) {					
					...
					let lifeCycle = LifeCycleObj[stage];
			...
			async function compileInstance(pidapx, inst, saveRoot = false) {
				...
				let mod;
				let ents = [];
				let entityLog = new Log(logger, modnam, __options);
				...
				for (let par of ents) {
					entsPromise.push((async function () {
						...
						EntCache[par.Pid] = new Volatile(new Entity(Nxs, ImpCache[impkey], par, entityLog));
