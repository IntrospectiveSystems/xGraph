...
module.exports = class CacheInterface {
	...
	constructor(__options) {
		...
		let backupLog = {
			d: _ => _, v: _ => _, i: _ => _, w: _ => _, e: _ => _, x: _ => _
		};
		this.log = __options.log || backupLog;
	...
	async addModule(moduleType, moduleZip64) {
		return new Promise(async (resolveModule, rejectModule) => {
			...
			try {
				moduleZip = Buffer.from(moduleZip64, 'base64');
			} catch(e) {
				log.e(`Failed to find module zip for module ${moduleType}`);
				rejectModule(e);
			}
			...
			try { fs.mkdirSync(cachedMod); } catch (e) { log.x(`${cachedMod} path already exists`); }
			try { fs.mkdirSync(libdir); } catch (e) { log.x(`${cachedMod} path already exists`); }
			...
			if ('package.json' in zipObj.files) {
				...
				log.x(`Installing dependencies for ${moduleType}`);
				...
				try {
					await new Promise((resolve, reject) => {
						if (this.checkFlag('node_modules')) {							
							// Enable manual application level node_modules
							resolve();
							return;
						}
						...
						npmInstallProcess.stdout.on('error', e => log.x('stdout/err: ' + e));
						npmInstallProcess.stderr.on('error', e => log.x('stderr/err: ' + e));
						...
						npmInstallProcess.on('exit', function (code) {
							if (code == 0) {
								// log.x(`${moduleType}: dependencies installed correctly`);
							} else {
							...
							// log.timeEnd(Path.resolve(libdir));
				...
				log.x(`no dependencies to install for ${moduleType}`);
				// resolve();
			}
		...
		function remDir(path) {
			return (new Promise(async (resolve, _reject) => {
				if (fs.existsSync(path)) {
					...
					for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
						promiseArray.push(new Promise(async (resolve2, _reject2) => {
							...
							if (fs.lstatSync(curPath).isDirectory()) {
								...
								// delete file
								log.x('Removing Entity ', files[fileIndex].split('.')[0]);
								fs.unlinkSync(curPath);
								resolve2();
							}
						}));
					...
					log.x('Removing Module Directory ', path);
					fs.rmdirSync(path);
					resolve();
				} else {
					log.x('trying to remove nonexistant path ', path);
					resolve();
				}
			}));
		}
	...
	async loadCache() {
		...
		let apexIndex = {}, entIndex = {}, cacheInterface = {};
		...
		function parseMod(folder) {
			...
			for (let ifile = 0; ifile < instancefiles.length; ifile++) {
				...
				if (fs.lstatSync(path).isDirectory()) {
					...
					for (let key in instJson) {
						if (key.startsWith("$")) {
							let stage = instJson[key].split(".")[0];
							//log.i(`Adding ${key} to ${stage}`);
							if (!(stage in cacheInterface)) {
								cacheInterface[stage] = [];
							}
							cacheInterface[stage][file] = instJson[key];
						}
					}				
		...
		cacheInterface.apexIndex = apexIndex;
	...
	loadDependency(moduleType, moduleName) {
		...
		if (version < ver130) {
			...
			if (this.checkFlag('node_modules')) {
				nodeModulesPath = Path.join(this.checkFlag('node_modules'), 'node_modules', moduleName);
			} else {
		...
		try {
			...
			try {
				...
				log.x(fs.existsSync(nodeModulesPath));
				log.x(moduleType, moduleName);
				log.x(nodeModulesPath.split(Path.sep).join('\n> '));
	...
	async createInstance(inst, pidapx = this.genPid()) {		
		...
		log.x('createInstance', pidapx, JSON.stringify(inst, null, 2));
		...
		log.x('parseMacros', pidapx, JSON.stringify(inst, null, 2));
		...
		if (!('Apex' in schema)) {
			log.x('keys in schema.json');
			log.x(Object.keys(schema).join('\r\n'));
